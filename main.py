...
OBSERVATION_TIMES = np.sort(
    rng.uniform(0, 24, N_OBSERVATIONS)
)  # NumPy array of timestamps e.g. [0.13, 0.79, 1.24, 1.90, 2,43]
DATA = np.zeros(
    N_OBSERVATIONS
)  # NumPy array of zeroes, to become an array of values from two Normal distributions
...
INITIAL_HYPOTHESIS_MU1 = (
    DATA[OBSERVATION_TIMES < INITIAL_HYPOTHESIS_TAU].mean()
    if np.any(OBSERVATION_TIMES < INITIAL_HYPOTHESIS_TAU)
    else 15.0
)
INITIAL_HYPOTHESIS_MU2 = (
    DATA[OBSERVATION_TIMES >= INITIAL_HYPOTHESIS_TAU].mean()
    if np.any(OBSERVATION_TIMES >= INITIAL_HYPOTHESIS_TAU)
    else 15.0
)
...
def save_observed_data_json(observation_times, data):
    """Save observed times and oxygen levels to JSON."""
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    output_path = OUTPUT_DIR / "observed.json"

    payload = {
        "observations": [
            {
                "time_hours": float(time),
                "oxygen_level_mg_per_l": float(oxygen_level),
            }
            for time, oxygen_level in zip(observation_times, data)
        ]
    }
...
def metropolis_hastings():
    ...
    for i in range(N_SAMPLES + BURN_IN_ITERATIONS):
        ...
        proposed_log_post = log_posterior(
            tau_proposal, mu1_proposal, mu2_proposal, DATA
        )
        ...
        if np.log(rng.random()) < log_ratio:
            tau_current, mu1_current, mu2_current = (
                tau_proposal,
                mu1_proposal,
                mu2_proposal,
            )
...
def main():
    ...
    print(
        f"There was a change-point in dissolved oxygen levels (mg/L) in stream water observed during a 24 hour period"
    )
    print()
    print(
        f"  {TRUE_TAU}h                                       -- the change-point time [unknown]"
    )
    print(
        f"  Normal({TRUE_MU1} mg/L, {TRUE_SIGMA}² mg/L²)      -- the distribution of oxygen levels before τ [only sigma is known]"
    )
    print(
        f"  Normal({TRUE_MU2} mg/L, {TRUE_SIGMA}² mg/L²)      -- the distribution of oxygen levels after τ [only sigma is known]"
    )
    print(
        f"  {TRUE_EFFECT_SIZE:.1f} mg/L                  -- the effect-size (change in mean oxygen levels) [unknown]"
    )
    print(f"""
    ...
    print(
        f"Observation times range from {OBSERVATION_TIMES[0]:.2f}h to {OBSERVATION_TIMES[-1]:.2f}h."
    )
    print(f"Oxygen levels range from {DATA.min():.1f}mg/L to {DATA.max():.1f}mg/L.")
    observed_output_path = save_observed_data_json(OBSERVATION_TIMES, DATA)
    print(f"Observed data saved to {observed_output_path}")
    print()
    print()
    ...
    print("Parameters:")
    print(
        "  τ  ~ Uniform(0, 24)         -- the change-point time, continuous on [0, 24] hours"
    )
    ...
    print(
        f"  Effect size: {effect_size.mean():.1f} mg/L         (true effect size: {TRUE_EFFECT_SIZE:.1f} mg/L)"
    )
    print()
    ...
    print(
        f"  μ₁: [{np.percentile(samples_mu1, 2.5):.1f}, {np.percentile(samples_mu1, 97.5):.1f}] mg/L"
    )
    print(
        f"  μ₂: [{np.percentile(samples_mu2, 2.5):.1f}, {np.percentile(samples_mu2, 97.5):.1f}] mg/L"
    )
    print(
        f"  Effect size: [{np.percentile(effect_size, 2.5):.1f}, {np.percentile(effect_size, 97.5):.1f}]"
    )
    print()
    # Some probability queries
    prob_afternoon = (samples_tau > 12).mean()
    print(
        f"Probability the change happened in the afternoon (after 12:00): {prob_afternoon:.1%}"
    )
    ...
    ax.axvline(
        ...
        linestyle="-",
        linewidth=2,
        label=f"Estimated = {samples_tau.mean():.2f}h",
    )
    ax.set_xlabel("Change-point τ (hours)")
    ax.set_ylabel("Posterior density")
    ax.set_title("When did the change happen?")
    tau_margin = max(
        0.5, (np.percentile(samples_tau, 99) - np.percentile(samples_tau, 1)) * 0.3
    )
    ax.set_xlim(
        np.percentile(samples_tau, 1) - tau_margin,
        np.percentile(samples_tau, 99) + tau_margin,
    )
    ax.legend(fontsize=8)
    ...
    ax.axvline(
        ...
        linestyle="-",
        linewidth=2,
        label=f"Estimated = {samples_mu1.mean():.1f}",
    )
    ...
    ax.axvline(
        ...
        linestyle="-",
        linewidth=2,
        label=f"Estimated = {samples_mu2.mean():.1f}",
    )
    ax.set_xlabel("μ₂ (mg/L)")
    ax.set_ylabel("Posterior density")
    ...
    ax.axhline(
        ...
        linewidth=1.5,
        alpha=0.8,
        label=f"True effect = {TRUE_EFFECT_SIZE:.0f} mg/L",
    )
    ax.axvline(
        samples_tau.mean(),
        color="green",
        ...
        linewidth=1.5,
        alpha=0.8,
        label=f"Est. τ = {samples_tau.mean():.2f}h",
    )
    ax.axhline(
        effect_size.mean(),
        color="green",
        ...
        linewidth=1.5,
        alpha=0.8,
        label=f"Est. effect = {effect_size.mean():.1f} mg/L",
    )
    ax.plot(
        samples_tau.mean(),
        effect_size.mean(),
    ...
    tau_margin_j = max(
        0.5, (np.percentile(tau_sub, 99) - np.percentile(tau_sub, 1)) * 0.3
    )
    eff_margin_j = max(
        1.0, (np.percentile(eff_sub, 99) - np.percentile(eff_sub, 1)) * 0.3
    )
    ax.set_xlim(
        np.percentile(tau_sub, 1) - tau_margin_j,
        np.percentile(tau_sub, 99) + tau_margin_j,
    )
    ax.set_ylim(
        np.percentile(eff_sub, 1) - eff_margin_j,
        np.percentile(eff_sub, 99) + eff_margin_j,
    )
